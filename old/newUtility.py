...
class NewEntities():
    ...
    def __str__(self):
        return 'Current "%s" will perform, "%s" on the %s using port %s thru %s, execute %s times with interval %s, store in %s... rest list are: %s' % (
            self.operation, self.userName, self.ip, self.port, self.connWay, self.times, self.interval, self.filePath,
            self.restList)
...
class Connector():
    ...
    def sftpUpload(self, localPath, remotePath):
        ...
        try:
            ...
            # print 'Uploading.....'
            widgets = ['Uploading ', localPath, ' : ', Percentage(), ' ', Bar(marker='#', left='[', right=']'), ' ',
